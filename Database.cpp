...
namespace neztu
{
    ...
    // g++ won't let me make these function-local - not sure if I'm doing
    ...
    public:
        ...
        void operator()(argument_type &T)
        {
            pqxx::result R = T.exec(
                std::string("SELECT * FROM \"Tracks\" WHERE \"TrackId\"=")
                + m_trackId
                );
            if (R.empty())
            {
    ...
    public:
        explicit GetRandomTracksFunctor(
    ...
    public:
        ...
        {
            T.exec(
                std::string("INSERT INTO \"Votes\" VALUES('")
                + T.esc(m_username) + "', " + m_trackId + ")"
                );
        }
    };
    ...
    public:
        ...
        {
            T.exec(
                std::string("INSERT INTO \"History\" VALUES('")
                + T.esc(m_username) + "', " + m_trackId + ")"
                );
        }
    };
    ...
    public:
        ...
        {
            T.exec(
                std::string("DELETE FROM \"Votes\" WHERE \"UserName\"='")
                + T.esc(m_username) + "' AND \"TrackId\"=" + m_trackId
                );
        }
    };
    ...
    public:
        explicit SwapVotesFunctor(
            const std::string &username,
            unsigned int trackId1,
            unsigned int trackId2
            ) :
            m_username(username)
        {
        ...
        void operator()(argument_type &T)
        {
            ...
            pqxx::result R = T.exec(
                std::string(
                    "SELECT \"Timestamp\" FROM \"Votes\" WHERE \"UserName\"='"
                    )
                + T.esc(m_username) + "' AND \"TrackId\"=" + m_trackId1
                );
            if (R.empty())
            {
            ...
            R = T.exec(
                ...
                + T.esc(m_username) + "' AND \"TrackId\"=" + m_trackId2
                );
            if (R.empty())
            {
                throw std::runtime_error("could not get timestamp");
    ...
    public:
        ...
        void operator()(argument_type &T)
        {
            pqxx::result R = T.prepared("current").exec();
            if (R.empty())
            {
    ...
    public:
        ...
        {
            pqxx::result R = T.exec(
                std::string("SELECT * FROM \"Tracks\" WHERE \"Filename\"='")
                + T.esc(m_filename) + "'"
                );
            if (R.empty())
            {
                // XXX: Blech.  For parity with the other GetTrack, this should
                // throw.  However, this function is primarily used by the
                // TagReader, and this would amount to a throw in the common
    ...
    public:
        ...
        void operator()(argument_type &T)
        {
            pqxx::result R = T.prepared("search")
                (m_title)
                (m_artist)
                (m_album)
                .exec();
    ...
    public:
        ...
        void operator()(argument_type &T)
        {
            pqxx::result R = T.prepared("getvotes").exec();
    ...
    public:
        explicit GetVotesForUserFunctor(
        ...
        void operator()(argument_type &T)
        {
            pqxx::result R = T.prepared("getvotesforuser")(m_username).exec();
            if (R.empty())
            {
                m_votes->resize(0);
            }
        ...
        m_conn(config.GetDatabaseConnectionString())
    {
        ...
        m_conn.prepare(
            ...
            ("timestamp", prepare::treat_string)
            ("varchar", prepare::treat_string)
        ...
        m_conn.prepare(
            ...
            ", \"DiscNumber\", \"TrackNumber\", \"Length\", \"Uploader\")"
        ...
        m_conn.prepare(
            "getbyfilename",
            "SELECT * FROM \"Tracks\" WHERE \"Filename\" = $1"
            )
            ("varchar", prepare::treat_string);
        ...
        m_conn.prepare(
            ...
            "SELECT * FROM \"History\""
            " LEFT JOIN \"Tracks\""
            " ON \"History\".\"TrackId\"=\"Tracks\".\"TrackId\""
        ...
        m_conn.prepare(
            ...
            "SELECT * FROM \"Votes\""
            " LEFT JOIN \"Tracks\""
            " ON \"Votes\".\"TrackId\"=\"Tracks\".\"TrackId\""
            " ORDER BY \"Timestamp\""
            );
